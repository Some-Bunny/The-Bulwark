...
namespace ItemAPI
{
    public static class CompanionBuilder
    {
        ...
        public static GameObject BuildPrefab(string name, string guid, string defaultSpritePath, IntVector2 hitboxOffset, IntVector2 hitBoxSize)
        {
            ...
            var sprite = SpriteBuilder1.SpriteFromResource(defaultSpritePath, prefab).GetComponent<tk2dSprite>();
        ...
        public static tk2dSpriteAnimationClip BuildAnimation(AIAnimator aiAnimator, string name, string spriteDirectory, int fps)
        {
            ...
            if (!collection)
                collection = SpriteBuilder1.ConstructCollection(aiAnimator.gameObject, $"{aiAnimator.name}_collection");
            ...
            for (int i = 0; i < resources.Length; i++)
            {
                ...
                {
                    indices.Add(SpriteBuilder1.AddSpriteToCollection(resources[i], collection));
                }
            }
            tk2dSpriteAnimationClip clip = SpriteBuilder1.AddAnimation(aiAnimator.spriteAnimator, collection, indices, name, tk2dSpriteAnimationClip.WrapMode.Loop);
            clip.fps = fps;
            return clip;
        }
